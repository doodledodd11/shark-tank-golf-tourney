import { getActiveTournament, getAllPlayers } from "@/lib/data";
    ...
    <div>
      ...
      <div className="mt-6 overflow-x-auto rounded-2xl border border-stone-200 bg-white shadow-sm">
        <div className="min-w-[880px]">
          <div className="grid grid-cols-12 gap-2 border-b border-stone-200 bg-stone-50 px-3 py-2 text-xs font-semibold uppercase tracking-wide text-ink-700/50">
          ...
          {players.map((player) => (
            <PlayerRow key={player.id} player={player} canDelete={canDelete} />
          ))}
        </div>
      </div>
    </div>
