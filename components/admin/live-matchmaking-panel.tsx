...
import { cancelMatchmaking } from "@/lib/actions/matchmaking";

export function LiveMatchmakingAdminPanel({
  roundId,
  roundName,
  teams,
  statusLabel,
  ...
  function handleCancel() {
    setError(null);
    if (confirm(`Cancel matchmaking for ${roundName}? Every match built so far will be deleted, but the locked twosomes stay intact.`)) {
      startTransition(async () => {
        const result = await cancelMatchmaking(roundId);
  ...
  return (
    <div className="rounded-2xl border border-gold-400/50 bg-gold-50/40 p-5">
      <div className="flex flex-wrap items-start justify-between gap-3">
        ...
        >
          {pending ? "Canceling…" : "Cancel Matchmaking"}
        </button>
      </div>
      {error && <p className="mt-2 text-sm font-medium text-red-600">{error}</p>}
      <p className="mt-1 text-sm text-ink-700/60">
        Captains alternate announcing a twosome and picking an opponent for it. Share each captain&apos;s link
        below.
      </p>
          ...
          team.captainAccessToken ? (
            ...
            >
              <div className="min-w-0">
                ...
                <p className="truncate text-xs text-ink-700/50">/matchmaking/{roundId}/{team.captainAccessToken}</p>
              </div>
              <CopyLinkButton path={`/matchmaking/${roundId}/${team.captainAccessToken}`} />
            </div>
          ) : null,
        )}
      </div>
